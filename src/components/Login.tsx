...
export default function Login() {
  ...
  const handleSendOtp = async (emailToSend?: string) => {
    const targetEmail = emailToSend || email.trim();
    setMessage('');

    // If we're sending OTP for a specific email (from saved users), update the email state
    if (emailToSend && emailToSend !== email) {
      setEmail(emailToSend);
      setCurrentEmailForOtp(emailToSend);
    } else {
      setCurrentEmailForOtp(targetEmail);
    }

    try {
      console.log('Sending OTP to:', targetEmail);
      await loginWithOtp(targetEmail);
      ...
      // Switch to OTP method if not already selected
      setAuthMethod('otp');
    } catch (error: any) {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const emailToVerify = currentEmailForOtp || email.trim();
      console.log('Verifying OTP for email:', emailToVerify);
      await verifyOtp(emailToVerify, otp.trim());
      setMessage('Login successful!');
    } catch (error: any) {
  ...
  const handleSelectUser = (selectedEmail: string) => {
    console.log('Selecting user:', selectedEmail);
    setEmail(selectedEmail);
    // Switch to OTP method when selecting a user from the list
    setAuthMethod('otp');
    setShowOtpInput(false);
    setMessage('');
  };
        ...
        <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8">
          ...
          {authMethod === 'otp' && (
            <div className="space-y-6">
              {!showOtpInput ? (
                <form onSubmit={handleSendOtpForm} className="space-y-6">
                  <div>
